...
class FrameworkRecommendation(DB.Model):
    ...
    @classmethod
    def get_framework_recommendations(self, pipeline_id):
        # Check if there are any framework recommendations
        framework_recommendation_query = DB.session.query(FrameworkRecommendation).filter(
            FrameworkRecommendation.pipeline_id==pipeline_id
        )

        framework_recommendations_in_database = framework_recommendation_query.all()

        if not framework_recommendations_in_database:
            raise ApiError("Not available yet, try again later.", 204)

        return framework_recommendations_in_database